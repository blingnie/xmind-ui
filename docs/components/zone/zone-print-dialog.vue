  ...
  >
    <div class="flex h-full gap-3 p-3">
      ...
      >
        ...
        <pagination
          v-if="print.scaleMode.value === 'custom'"
          class="mt-5"
              ...
              <div v-if="print.scaleMode.value === 'custom'" class="ml-6 mt-2 flex items-start gap-2">
                <div class="flex flex-col items-center gap-1">
                  <measure-input
                    ...
                    measurement=""
                    class="w-16"
                    @input="print.updateColumns(parseInt($event))"
                  />
                ...
                <div class="flex flex-col items-center gap-1">
                  <measure-input
                    ...
                    measurement=""
                    class="w-16"
                    @input="print.updateRows(parseInt($event))"
                  />
...
import type { MindMap } from 'client/library/workbook-next'
import { useZonePrint } from 'app/composables/use-zone-print'
import { usePromiseHook } from 'client/utils/vue-utils'

type PaperSize = 'A4' | 'A3' | 'Letter' | 'Legal'

const props = defineProps<{
  workbookEditor: MindMap
  scopeZoneId: string
}>()
...
const workbookEditorRef = computed(() => props.workbookEditor)
const scopeZoneIdRef = computed(() => props.scopeZoneId)

const print = useZonePrint(workbookEditorRef, scopeZoneIdRef, localPreviewRef)
...
const handleLayoutChange = (item: { key: string }) => {
  print.orientation.value = item.key as 'landscape' | 'portrait'
}

const handleClose = () => {
  print.close()
  promiseHook.resolve(false)
}

const handlePrint = async () => {
  await print.executePrint()
  promiseHook.resolve(true)
  print.close()
}

const promiseHook = usePromiseHook()
