...
const { getColorGroups, getAliasTokens, getSpecificTokens, getAiSpecificTokens } = useTokens()

const activeTab = ref('Alias')
const tabs = ['Alias', 'Specific', 'AI Specific', 'Palette']

// ── Alias Token ──
const aliasTokens = getAliasTokens('Light')
const aliasGroups = computed(() => {
  const map = new Map<string, typeof aliasTokens>()
...
// ── Specific Token ──
const specificTokens = getSpecificTokens()
const specificGroups = computed(() => {
  const map = new Map<string, typeof specificTokens>()
...
// ── AI Specific Token ──
const aiSpecificTokens = getAiSpecificTokens()
const aiSpecificGroups = computed(() => {
  const map = new Map<string, typeof aiSpecificTokens>()
  for (const t of aiSpecificTokens) {
    const group = t.name.split('/')[0]
    if (!map.has(group)) map.set(group, [])
    map.get(group)!.push(t)
  }
...
<template>
  <div>
    ...
    <!-- Alias Token -->
    <div v-show="activeTab === 'Alias'" class="mt-4">
    ...
    <!-- Specific Token -->
    <div v-show="activeTab === 'Specific'" class="mt-4">
      <div v-for="{ group, tokens } in specificGroups" :key="group" class="mb-6">
        <p class="text-xs font-medium text-[var(--color-text-tertiary)] tracking-wider mb-2">{{ capitalize(group) }}</p>
        <div class="rounded-xl border border-[var(--color-border-translucent)] overflow-hidden divide-y divide-[var(--color-border-translucent)]">
          <div v-for="token in tokens" :key="token.varName" class="flex items-center gap-4 px-4 py-2.5 hover:bg-[var(--color-fill-secondary)] transition-colors">
            <div class="w-7 h-7 rounded-md border border-[var(--color-border-translucent)] shrink-0" :style="{ background: `var(${token.varName})` }" />
    ...
    <!-- AI Specific Token -->
    <div v-show="activeTab === 'AI Specific'" class="mt-4">
      <div v-for="{ group, tokens } in aiSpecificGroups" :key="group" class="mb-6">
        <p class="text-xs font-medium text-[var(--color-text-tertiary)] tracking-wider mb-2">{{ capitalize(group) }}</p>
        <div class="rounded-xl border border-[var(--color-border-translucent)] overflow-hidden divide-y divide-[var(--color-border-translucent)]">
          <div v-for="token in tokens" :key="token.varName" class="flex items-center gap-4 px-4 py-2.5 hover:bg-[var(--color-fill-secondary)] transition-colors">
            <div class="w-7 h-7 rounded-md border border-[var(--color-border-translucent)] shrink-0" :style="{ background: `var(${token.varName})` }" />
    ...
    <!-- Palette -->
    <div v-show="activeTab === 'Palette'" class="mt-4">
