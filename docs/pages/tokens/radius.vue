...
<template>
  <div class="px-10 py-8 max-w-5xl">
    ...
    <div class="grid grid-cols-4 gap-6">
      <button v-for="token in tokens" :key="token.varName" class="flex flex-col items-center gap-2 p-4 rounded-xl border border-[var(--color-border-translucent)] bg-[var(--color-fill-surfacebright)] hover:shadow-[var(--shadow-l2)] transition-shadow" @click="copy(`var(${token.varName})`)">
